...
new_features = pd.DataFrame([[6,85,66,7,7,90]],columns=features)
...
new_features = pd.DataFrame([[6,82,50,6,7,80],[7,80,60,4,5,50],[5,65,67,4,6,40],[6,78,80,7,5,85],[7,60,62,4,8,30]],columns=features)
...
plot_tree(model , filled=True , feature_names=features , class_names=["Failed","Passed"])

plt.title("Decision Tree Classifier")
plt.show()

# Accuracy is same by adding feature as it was 100 % from start

# Accuracy is 100% even if max_depth is None

# If training accuracy is 100% and testing is lower due to shufel or chances was not known to that part
